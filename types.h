...
// A collectiong of Isotopologues.  We may need to parse and define these a bit
// better
struct Isotopologue {
  std::string name;
...
class Species {
public:
  Species(std::string name, std::vector<Atom> atoms, std::vector<Bond> bonds,
	  std::vector<Angle> angles, std::vector<Torsion> torsions,
	  std::vector<Isotopologue> isos, std::vector<Site> sites,
	  std::vector<Forcefield> forcefields)
      : name_(name), atoms_(atoms), bonds_(bonds), angles_(angles),
	torsions_(torsions), isotopologues_(isos), sites_(sites),
	forcefields_(forcefields) {}
  std::string name_;
...
public:
  PairPotentialParameters(std::string name, std::string element,
			  double strength, PP type)
      : name_(name), element_(element), strength_(strength), type_(type){};
  friend std::ostream &operator<<(std::ostream &out,
				  const PairPotentialParameters &p);
